...
1. **Criar um builder com `docker-container`**:

```
docker buildx create --name meu-builder --driver docker-container --use
```

1. **Inicializar o builder**:

```
docker buildx inspect --bootstrap
```

1. **Verificar se o builder está ativo**:
...
1. **Verificar a imagem no registry**: As imagens para ambas as arquiteturas estarão disponíveis.

### **Exemplo 3: Gerenciamento de Builders e Contextos**

1. **Listar builders e contextos**:
...
1. **Remover um builder associado a um contexto**: Primeiro, remover o contexto:
